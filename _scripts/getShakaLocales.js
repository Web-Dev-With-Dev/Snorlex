...
/**
 * Maps the shaka locales to Snorlex's active ones
 * This allows us to know which locale files are actually needed
 * and which shaka locale needs to be activated for a given Snorlex one.
 * @param {Set<string>} shakaLocales
 * @param {string[]} snorlexLocales
 */
function getMappings(shakaLocales, snorlexLocales) {
  /**
   * @type {[string, string][]}
   * Using this structure as it gets passed to `new Map()` in the player component
   * The first element is the Snorlex locale, the second one is the shaka-player one
   */
  const mappings = []

  for (const locale of snorlexLocales) {
  ...
  mappings.push(
    ...
    // not sure why we have pt, pt-PT and pt-BR in the Snorlex locales
...
function getShakaLocales() {
  ...
  /** @type {string[]} */
  const snorlexLocales = JSON.parse(readFileSync(join(__dirname, '../static/locales/activeLocales.json'), 'utf-8'))

  const mappings = getMappings(shakaLocales, snorlexLocales)
