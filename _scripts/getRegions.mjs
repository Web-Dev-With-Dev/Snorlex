 ...
 *
 * It tries to map every active Snorlex language (static/locales/activelocales.json)
...
// map Snorlex languages to their YouTube equivalents
...
console.log("Active Snorlex languages that aren't available on YouTube:")
console.log(activeLanguages.filter(lang => !foundLanguageNames.includes(lang)).sort())

console.log("YouTube languages that don't have an equivalent active Snorlex language:")
...
for (const { youTube, snorlex } of languagesToScrape) {
  const response = await scrapeLanguage(youTube)

  processGeolocations(snorlex, youTube, response)
}
...
/**
 * @param {string} snorlexLanguage
 * @param {string} youTubeLanguage
 * @param {import('youtubei.js').ApiResponse} response
 */
function processGeolocations(snorlexLanguage, youTubeLanguage, response) {
  ...
  const normalisedSnorlexLanguage = snorlexLanguage.replace('_', '-')
  ...
  localeSet.add(normalisedSnorlexLanguage)
  localeSet.add(youTubeLanguage)
  localeSet.add(normalisedSnorlexLanguage.split('-')[0])
  ...
  writeFileSync(`${STATIC_DIRECTORY}/geolocations/${snorlexLanguage}.json`, JSON.stringify(output))
}
