...
const exportSubscriptionsPromptNames = computed(() => {
  const exportSnorlex = t('Settings.Data Settings.Export Snorlex')
...
async function importSubscriptions() {
  ...
  if (filename.endsWith('.csv')) {
    ...
    importSnorlexSubscriptions(content)
 ...
 * @param {any[]} oldData
 */
function convertOldSnorlexFormatToNew(oldData) {
  const convertedData = []
 ...
 */
function importSnorlexSubscriptions(textDecode) {
  ...
  if (firstEntry.channelId && firstEntry.channelName && firstEntry.channelThumbnail && firstEntry._id && firstEntry.profile) {
    // Old Snorlex subscriptions format detected, so convert it to the new one:
    textDecode = convertOldSnorlexFormatToNew(textDecode)
  }
...
/**
 * @param {'snorlex' | 'youtubenew' | 'youtube' | 'youtubeold' | 'newpipe' | 'close' | null} option
 */
...
async function exportSnorlexSubscriptions() {
  const subscriptionsDb = profileList.value.map((profile) => {
    return JSON.stringify(profile)
  }).join('\n') + '\n'// a trailing line is expected
  const dateStr = getTodayDateStrLocalTimezone()
  const exportFileName = 'snorlex-subscriptions-' + dateStr + '.db'

  await promptAndWriteToFile(
    exportFileName,
    subscriptionsDb,
    t('Settings.Data Settings.Subscription File'),
    'application/x-snorlex-db',
    '.db',
...
const exportWatchSearchHistoryPromptNames = computed(() => [
  `${t('Settings.Data Settings.Export Snorlex')} (.db)`,
...
async function importWatchHistory() {
  ...
  if (filename.endsWith('.db')) {
    importSnorlexWatchHistory(content.split('\n'))
  } else if (filename.endsWith('.json')) {
 ...
 * @param {string[]} textDecode
 */
async function importSnorlexWatchHistory(textDecode) {
  textDecode.pop()
  ...
  textDecode.forEach((history) => {
    ...
    if (missingKeys.length > 0) {
      ...
      // Snorlex history export does not have this data if the video was marked as watched manually, setting default value
...
/**
 * @param {'snorlex' | 'youtube' | null} option
 */
...
async function exportSnorlexWatchHistory() {
  const historyDb = historyCacheSorted.value.map((historyEntry) => {
    return JSON.stringify(historyEntry)
  }).join('\n') + '\n'
  const dateStr = getTodayDateStrLocalTimezone()
  const exportFileName = 'snorlex-watch-history-' + dateStr + '.db'
...
async function exportPlaylists() {
  const dateStr = getTodayDateStrLocalTimezone()
  const exportFileName = 'snorlex-playlists-' + dateStr + '.db'
...
async function importSearchHistory() {
  ...
  if (filename.endsWith('.db')) {
    importSnorlexSearchHistory(content.split('\n'))
  } else if (filename.endsWith('.json')) {
 ...
 * @param {string[]} textDecode
 */
async function importSnorlexSearchHistory(textDecode) {
  textDecode.pop()
...
async function exportSnorlexSearchHistory() {
  const historyDb = searchHistoryEntries.value.map((entry) => {
    return JSON.stringify(entry)
  }).join('\n') + '\n'
  const dateStr = getTodayDateStrLocalTimezone()
  const exportFileName = 'snorlex-search-history-' + dateStr + '.db'
...
async function exportSettings() {
  const settingDb = Object.entries(store.state.settings)
    ...
    .join('\n') + '\n'
  const dateStr = getTodayDateStrLocalTimezone()
  const exportFileName = 'snorlex-settings-' + dateStr + '.db'
