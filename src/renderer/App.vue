...
async function checkForNewUpdates() {
  ...
  try {
    ...
    let changelog = (json[0].body || '')
      ...
      .replaceAll(/https:\/\/github\.com\/Web-Dev-With-Dev\/Snorlex\/pull\/(\d+)/g, '[#$1]($&)')
    ...
    console.error('errored while checking for updates', 'https://api.github.com/repos/Web-Dev-With-Dev/Snorlex/releases?per_page=1', error)
  }
}
...
function openDownloadsPage() {
  openExternalLink('https://snorlexapp.io#download')
 ...
 * Cancels the drag operation if the URL is Snorlex specific and cannot be transformed e.g. user playlist URLs
...
function handleDragStart(event) {
  ...
  if (transformed) {
    ...
    // Cancel the drag operation for Snorlex specific URLs that cannot be transformed such as user playlist URLs
    event.preventDefault()
