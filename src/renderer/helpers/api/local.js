...
async function createInnertube({ withPlayer = false, location = undefined, safetyMode = false, clientType = undefined, generateSessionLocally = true, fetchFunc = null } = {}) {
  ...
  return await Innertube.create({
    ...
    // That behavior is highly undesirable for Snorlex, as we want to create a new session every time to limit tracking.
...
async function getWatchHTMLWatchPage(videoId, fetchFunc) {
  ...
  if (sessionStorage.getItem('playerHtmlHomepageFallback') === '1') {
    ...
    try {
      ...
      // Fall back to the home page for the rest of the session/until Snorlex is restarted
 ...
 * @property {string} snorlexUrl deciphered streaming URL, stored in a custom property so the DASH manifest generation doesn't break
 *
...
async function decipherFormats(formats, player) {
  for (const format of formats) {
    ...
    format.snorlexUrl = await format.decipher(player)
  }
}
...
export function parseLocalCommunityPosts(posts) {
  ...
  // see: https://github.com/SnorlexApp/Snorlex/issues/3252#issuecomment-1546675781
