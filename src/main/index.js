...
import { isSnorlexUrl } from './utils'
...
function runApp() {
  if (process.platform === 'win32') {
    app.setAppUserModelId('io.snorlex.Snorlex')
  }

  /** @type {Set<string>} */
  const ALLOWED_RENDERER_FILES = process.env.NODE_ENV === 'production'
    // __SNORLEX_ALLOWED_PATHS__ is replaced by the injectAllowedPaths.mjs script
    ? new Set(__SNORLEX_ALLOWED_PATHS__)
    : new Set()
  ...
  app.removeAsDefaultProtocolClient('snorlex')

  // If we are running a non-packaged version of the app && on windows
  if (process.env.NODE_ENV === 'development' && process.platform === 'win32') {
    // Set the path of electron.exe and your app.
    // These two additional parameters are only available on windows.
    app.setAsDefaultProtocolClient('snorlex', process.execPath, [path.resolve(process.argv[1])])
  } else {
    app.setAsDefaultProtocolClient('snorlex')
  }
    ...
    app.on('second-instance', async (_, commandLine, __) => {
      ...
      if (typeof commandLine !== 'undefined') {
        ...
        const readyHandler = (event) => {
          if (isSnorlexUrl(event.senderFrame.url)) {
  ...
  app.on('ready', async (_, __) => {
    ...
    // Snorlex only needs a few permissions, so we reject requests for other permissions
    // and reject all requests on non-Snorlex URLs.
    //
    // Snorlex needs the following permissions:
    ...
    session.defaultSession.setPermissionCheckHandler((webContents, permission, requestingOrigin, details) => {
      if (!isSnorlexUrl(requestingOrigin)) {
        return false
      }
    ...
    session.defaultSession.setPermissionRequestHandler((webContents, permission, callback, details) => {
      if (!isSnorlexUrl(webContents.getURL())) {
    ...
    session.defaultSession.on('file-system-access-restricted', (event, details, callback) => {
      if (!isSnorlexUrl(details.origin)) {
        // eslint-disable-next-line n/no-callback-literal
        callback('deny')
        return
      }
  ...
  async function createWindow(
    ...
    const windowIcon = process.env.NODE_ENV === 'development'
      ? path.join(__dirname, process.platform === 'win32' ? '../../_icons/icon.ico' : '../../_icons/iconColor.png')
      : path.join(__dirname, process.platform === 'win32' ? '../_icons/icon.ico' : '../_icons/iconColor.png')
    ...
    newWindow.webContents.setWindowOpenHandler((details) => {
      ...
      // Only handle valid URLs that came from a Snorlex page
      if (url !== null && isSnorlexUrl(newWindow.webContents.getURL())) {
        if (isSnorlexUrl(url)) {
          createWindow({
            replaceMainWindow: false,
            showWindowNow: true,
  ...
  ipcMain.on(IpcChannels.SET_WINDOW_TITLE, (event, title) => {
    if (isSnorlexUrl(event.senderFrame.url) && typeof title === 'string') {
  ...
  function relaunch() {
    if (process.env.NODE_ENV === 'development') {
      app.exit(parseInt(process.env.SNORLEX_RELAUNCH_EXIT_CODE))
      return
    }
  ...
  nativeTheme.on('updated', () => {
    ...
    allWindows.forEach((window) => {
      if (isSnorlexUrl(window.webContents.getURL())) {
  ...
  ipcMain.handle(IpcChannels.GET_NAVIGATION_HISTORY, ({ senderFrame, sender }) => {
    if (!isSnorlexUrl(senderFrame.url)) {
      return
    }
  ...
  ipcMain.handle(IpcChannels.WRITE_TO_DEFAULT_FOLDER, async (event, filename, arrayBuffer) => {
    if (
      !isSnorlexUrl(event.senderFrame.url) ||
  ...
  function syncOtherWindows(channel, event, payload) {
    const otherWindows = BrowserWindow.getAllWindows().filter((window) => {
      return window.webContents.id !== event.sender.id && isSnorlexUrl(window.webContents.getURL())
    })
  ...
  /*
   * Callback when processing a snorlex:// link (macOS)
   */
   ...
   * Remove snorlex:// protocol if present
   */
  ...
  function baseUrl(arg) {
    let newArg = arg.replace('snorlex://', '')
    // add support for authority free url
      .replace('snorlex:', '')

    // fix for Qt URL, like `snorlex://https//www.youtube.com/watch?v=...`
    // For details see https://github.com/SnorlexApp/Snorlex/pull/3119
  ...
  function navigateTo(path, browserWindow) {
    if (browserWindow == null || !isSnorlexUrl(browserWindow.webContents.getURL())) {
      return
    }
